...
class ProductController extends Controller
{
    ...
    public function addtocart(Request $request, $id = null)
    {
        ...
        if ($countProducts > 0) {
            ...
            $getSKU = Transaction::select('sku', 'price')->where(['product_id' => $data['product_id']])->first();
    ...
    {
        // get main categories with sub categories
        $categories = Category::with('child_categories.products')->where(['parent_id' => 0])->get();
        $products = Product::with('transactions', 'fav')->inRandomOrder()->where(['status' => 'active', 'feature_item' => '1'])->paginate(3);

        return view('front.categories.all', compact('categories', 'products'));
    }
}